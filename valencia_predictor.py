...
class ValenciaPredictor(object):
    ...
    def __init__(self, vac_model_weights = None, model_preset = ModelPreset.VAC_H7_SUS, load_default_model=False,path_with_data = None):
    #def __init__(self):
        ...
        if(vac_model_weights is not None):
            ...
            self.model.load_weights(path_with_data)#[NEW] Ahora los pesos se cargan a partir de la ruta que recibimos como parámetro
    ...
    def predict_from_df(self,
       ...
       # """
            ...
            zn_g = self.df['kk'][self.df.GeoID == g]
            zn_date = zn_g[self.df.Date < current_date]
            #print(zn_date)
            zn = np.array(zn_date)
            #[NEW] 01/03/2023
            zn = np.array(compute_7days_mean(zn))# Hace la media de los ultimos 7 dias
            rec_g = self.df['recuperados'][self.df.GeoID == g] 
            rec_date = rec_g[self.df.Date < current_date]
            rec = np.array(rec_date)
            
            # [NEW] Ahora controlamos el ratio inmunidad de la población
            rn = np.array(compute_rns(self,self.model_preset,past_cum_cases, zn, pop_size,current_date,g, past_prop_immunized,rec))
            ...
            while current_date <= end_date:
                ...
                if self.model_preset == ModelPreset.VAC_NONE or self.model_preset == ModelPreset.VAC_H7:
                    ...
                    #pred_cases = int(((((pred_rn * ((pop_size*(1-0.7*past_prop_immunized[-1]) - zn[-1] - 0.7* rec[-1]) / pop_size)) - 1.0) * WINDOW_SIZE * zn[-1])) + past_cases[-7])
                    # [NEW] 1/03/2023
                    # Vamos a sumar 14 dias a current_date para que nos de la fecha de la prediccion
                    df_g = self.df[self.df.GeoID == g]
                    vacunados_hasta_ese_dia = df_g[df_g.Date < current_date].VacunadosDiarios.sum()
                    # Cogemos los vacunanados desde el inicio hasta hoy - 14
                    vacunados_diez_dias = df_g[df_g.Date >= (current_date - np.timedelta64(14, 'D'))].DiezPorCiento.sum()
    ...
    def _prepare_dataframe(self) -> pd.DataFrame:
        ...
        df['SmoothNewDeaths'] = df.groupby('GeoID')['NewDeaths'].rolling(
            WINDOW_SIZE, center=False).mean().fillna(0).reset_index(0, drop=True)
        # hacer uan suma de todos lso infectados. Y cada 14 dias quitarle a la los infectados
        kk = list(df['ConfirmedCases'])
        kk_rec = [kk[i-14] if i >= 14 else 0 for i in range(len(kk))]
        df['recuperados'] = ((df['ConfirmedCases'].rolling(14).sum())/14).round(0)
        kk = [kk[i] - kk[i-14] if i >= 14 else 0 for i in range(len(kk))]
        df['kk'] = kk
        # [NEW] Vamos a implementar el modelo de vacunación de Pzifer
        # Para ello vamos a añadir al modelo 3 columnas nuevas
        ...
        vacunados_diarios = [x if x >= 0 else 0 for x in vacunados_diarios] 
        # Ponemos a 0 las vacunaciones de china
        diez_por_ciento = list(round((df['ProportionImmunized'] * df['Population']) * 0.1, 0)) # Número de población * % de vacunación diaria (%) * 0.1
        diez_por_ciento = list(pd.Series(diez_por_ciento).diff().fillna(0))
        # Eliminamos los valores negativos
        diez_por_ciento = [x if x >= 0 else 0 for x in diez_por_ciento]
        #[New] 07/03/2023 cambiamos el 25 al 30
        treinta_cinco_por_ciento = list(round((df['ProportionImmunized'] * df['Population']) * 0.25, 0)) # Número de población * % de vacunación diaria (%) * 0.35
        #treinta_cinco_por_ciento = list(round((df['ProportionImmunized'] * df['Population']) * 0.35, 0)) # Número de población * % de vacunación diaria (%) * 0.35
        treinta_cinco_por_ciento = list(pd.Series(treinta_cinco_por_ciento).diff().fillna(0))
        # Eliminamos los valores negativos
        treinta_cinco_por_ciento = [x if x >= 0 else 0 for x in treinta_cinco_por_ciento]
        sesenta_y_cinco_por_ciento = list(round((df['ProportionImmunized'] * df['Population']) * 0.65, 0)) # Número de población * % de vacunación diaria (%) * 0.65
        sesenta_y_cinco_por_ciento = list(pd.Series(sesenta_y_cinco_por_ciento).diff().fillna(0))
        # Eliminamos los valores negativos
        sesenta_y_cinco_por_ciento = [x if x >= 0 else 0 for x in sesenta_y_cinco_por_ciento]
        # Introducimos las columnas en el dataframe
        df['VacunadosDiarios'] = vacunados_diarios
        df['DiezPorCiento'] = diez_por_ciento
        df['TreintaCincoPorCiento'] = treinta_cinco_por_ciento
        df['SesentaYCincoPorCiento'] = sesenta_y_cinco_por_ciento
        # Idea de oscar de meter un nuevo dataframe con los datops que hemos generado
        #df_registro_akuna_vakuna = df[df[['Date', 'GeoID', 'VacunadosDiarios', 'DiezPorCiento', 'TreintaCincoPorCiento']]].copy()
...
# Train model
    ...
    def _permute_data(self, X_context, X_action, y, seed=301):
        ...
        #[NEW] 03/03/2023
        # Se ha elimnado el shuffle que hacia Eloy
...
def compute_rns(self,model_preset,casos_acumulados, zn, population, current_date,g, prop_immunized=None,rec=None):#[NEW] added new parameter prop_immunized
    '''
    ...
    for i in range(len(casos_acumulados)):
        if i == 0:
            ...
            if zn[i-1] == 0:
                ...
                #[NEW] 10/02/2023 added new parameter prop_immunized
                if model_preset == ModelPreset.VAC_H7:
                    ##[NEW] 15/02/2023
                    denom = (population*(1-0.7*prop_immunized[i]) - casos_acumulados[i-1] -0.7* rec[i]) * zn[i-1]  # En xprize utilizan casos_acumulados[i]
                    ...
                    vacunados_treinta_y_cinco = df_g[(df_g.Date >= (current_date - np.timedelta64(60, 'D'))) & (df_g.Date <= (current_date - np.timedelta64(14, 'D')))].TreintaCincoPorCiento.sum()
                    vacunados_cien_por_ciento = df_g[df_g.Date == current_date - np.timedelta64(61, 'D')].SesentaYCincoPorCiento.sum()
                    denom = int(((((((population - zn[-1] - vacunados_hasta_ese_dia + vacunados_diez_dias/14 + vacunados_treinta_y_cinco/46 + vacunados_cien_por_ciento) * zn[i-1]))))))
                    #print("denominador",denom)
                    #denom = (population - casos_acumulados[i-1] + prop_immunized[i]*(- PARTIAL_VAC_IMMUNIZATION_PROB) + rec[i]*(1-FULLY_VAC_IMMUNIZATION_PROB)) * zn[i-1]  # En xprize utilizan casos_acumulados[i]
                
                #print("denom",denom)
                #print("Population",population)
                #print("casos_acumulados[i-1]",casos_acumulados[i-1])
                #print("prop_immunized[i-1]",prop_immunized[i-1])
                #print("rec[i]",rec[i])
                #print("zn[i-1]",zn[i-1])

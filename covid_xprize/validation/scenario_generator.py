...
def generate_scenario(start_date_str, end_date_str, raw_df, countries=None, scenario="Freeze",
                        fixed_immunized_delta_per_day=None, immunization_vel_multiplier=None):
    """
    ...
    for country in ips_df.CountryName.unique():
        ...
        for region in all_regions:
            ...
            # Obtains reference values (for Freeze scenario)
            day_before_start = max(INCEPTION_DATE, current_date - np.timedelta64(1, 'D'))
            freeze_npis = list(ips_gdf[ips_gdf.Date == day_before_start][NPI_COLUMNS].values[0])

            # Obtains current proportion of immunized people (before the start date)
            current_prop_immunized = ips_gdf[ips_gdf.Date == day_before_start][NPI_COLUMNS[-1]].values[0]

            if fixed_immunized_delta_per_day is not None:
                immunization_vel = fixed_immunized_delta_per_day
            else:
                # Obtains vaccination velocity
                week_before_start = max(INCEPTION_DATE, day_before_start - np.timedelta64(7, 'D'))
                month_before_start = max(INCEPTION_DATE, day_before_start - np.timedelta64(28, 'D'))
                prop_immunized_week = ips_gdf[ips_gdf.Date == week_before_start][NPI_COLUMNS[-1]].values[0]
                prop_immunized_month = ips_gdf[ips_gdf.Date == month_before_start][NPI_COLUMNS[-1]].values[0]

                vel_immunized_week = (current_prop_immunized - prop_immunized_week) / 7
                vel_immunized_month = (current_prop_immunized - prop_immunized_month) / 28

                immunization_vel = vel_immunized_month
            
            if immunization_vel_multiplier is not None:
                immunization_vel = immunization_vel * immunization_vel_multiplier


            while current_date <= end_date:
                new_row = [country_name, region_name, current_date]
                current_prop_immunized = current_prop_immunized + immunization_vel
                current_prop_immunized = min(current_prop_immunized, MAX_IMMUNIZATION_PROB)
                if current_date < start_date:
                    ...
                    if scenario == "MIN":
                        npis = MIN_NPIS
                        npis[-1] = freeze_npis[-1] # Proportion of inmunized people is frozen
                    elif scenario == "MAX":
                        npis = MAX_NPIS
                    elif scenario == "Freeze":
                        npis = freeze_npis
                        npis[-1] = current_prop_immunized # Keeps immunization velocity
                    else:
